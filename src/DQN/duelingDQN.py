...
import math
epsilon_scheduled = lambda index: 0.0001 + (0.5 - 0.0001) * math.exp(-1. * index / 4000)


board = Board(epsilon_scheduled,algorithm='double-dqn',exp=0,batch_size=batch_size,update_freq=update_freq)
...
path_out = "/data/src/tensorboard/DQN/" + version
configure(path_out, flush_secs=5)

#Folder to store weights, if it doesn't exists create it
if not os.path.exists('model/'):
	os.mkdir('model/')
if not os.path.exists(os.path.join('model', version)):
	os.mkdir(os.path.join('model', version))
...
message = "Total time: {}".format((time()-start)/60)
print(message)
send(message)

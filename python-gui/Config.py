...
class Config():
	...
	def correctValues(self):
		...
		for f in [self.normal_reward, self.epsmax, self.epsmin, self.variance, self.pos, self.min_wall, self.max_wall]:
			if not all([a.lstrip('-').isdigit() for a in f.split('.')]):
				self.logs.append("Correct float {}. eg: 3.14".format(f))
				return False
		for i in [self.iterations, self.savefreq, self.batch_size, self.health, self.numwalls,  self.visibleRad, self.done_reward, self.edge_value, self.numAgents]:
			if not i.lstrip('-').isdigit():
				self.logs.append("Correct integer {}".format(i))
				return False
		if self.version == '':
	...
	def getJSONData(self):
		...
		data['done_reward'] = int(self.done_reward)
